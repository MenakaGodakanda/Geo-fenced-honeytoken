...
from ip_geolocation import IPGeolocationService, IPInfo

class HoneytokenDetector:
    """
    Enhanced geo-fenced honeytoken detection system with proxy/VPN detection
    """
    ...
    def is_ip_in_authorized_region(self, ip_location: IPInfo, metadata: Dict) -> bool:
        """
        Check if IP location is within authorized geographical region
        
        Args:
            ip_location (IPInfo): IP geolocation data
        ...
        ip_country_code = ip_location.country_code
        ...
        if lat_range and lon_range:
            ip_lat = ip_location.latitude
            ip_lon = ip_location.longitude
    ...
    def generate_alert(self, filename: str, ip_address: str, ip_location: IPInfo, 
                      metadata: Dict, scenario: str) -> Dict:
        """
        Generate enhanced security alert for honeytoken access with proxy/VPN detection
        ...
        threat_indicators = []
        if ip_location.is_proxy:
            threat_indicators.append("PROXY_SERVER")
        if ip_location.is_vpn:
            threat_indicators.append("VPN")
        if ip_location.is_tor:
            threat_indicators.append("TOR_NETWORK")
        if ip_location.is_hosting:
            threat_indicators.append("HOSTING_PROVIDER")
        ...
        alert = {
            ...
            "network_characteristics": {
                "is_proxy": ip_location.is_proxy,
                "is_vpn": ip_location.is_vpn,
                "is_tor": ip_location.is_tor,
                "is_hosting": ip_location.is_hosting,
                "isp": ip_location.isp
            },
            "risk_assessment": {
                "geo_risk": "LOW" if is_geo_valid else "HIGH",
                "access_risk": "HIGH",  # Always high for honeytoken access
                "anonymity_risk": "HIGH" if threat_indicators else "LOW",
                "overall_risk": "CRITICAL" if (not is_geo_valid or threat_indicators) else "HIGH"
            },
            "recommended_actions": [
                ...
                "Analyze for additional proxy/VPN usage" if threat_indicators else None
            ]
        }
        
        # Remove None from recommended actions
        alert['recommended_actions'] = [a for a in alert['recommended_actions'] if a is not None]
    ...
    def process_access_attempt(self, filename: str, ip_address: str) -> Dict:
        ...
        if ip_location.is_proxy or ip_location.is_vpn or ip_location.is_tor:
            print(f"   ⚠️  Anonymity Tool Detected: ", end="")
            tools = []
            if ip_location.is_proxy: tools.append("Proxy")
            if ip_location.is_vpn: tools.append("VPN")
            if ip_location.is_tor: tools.append("Tor")
            print(", ".join(tools))
    ...
    def display_alert(self, alert: Dict):
        ...
        print()
        print("🛡️  NETWORK CHARACTERISTICS:")
        net = alert['network_characteristics']
        print(f"   Proxy: {'✅ YES' if net['is_proxy'] else '❌ NO'}")
        print(f"   VPN: {'✅ YES' if net['is_vpn'] else '❌ NO'}")
        print(f"   Tor: {'✅ YES' if net['is_tor'] else '❌ NO'}")
        print(f"   Hosting Provider: {'✅ YES' if net['is_hosting'] else '❌ NO'}")
        
        if alert['threat_indicators']:
            print(f"   Threat Indicators: {', '.join(alert['threat_indicators'])}")
        ...
        print(f"   Anonymity Risk: {risk['anonymity_risk']}")
...
if __name__ == "__main__":
    ...
    # Test with a known VPN IP
    test_ip = "185.86.212.34"  # NordVPN
